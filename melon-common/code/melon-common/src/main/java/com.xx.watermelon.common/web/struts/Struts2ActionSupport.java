package com.zc.travel.common.web.struts;
...
import com.zc.travel.common.web.themes.dwz.DwzParam;
import com.zc.travel.common.web.utils.UdpGetClientMacAddr;
...
public class Struts2ActionSupport extends ActionSupport {
	...
	/**
	 * 根据参数key从HttpRequest中获取String类型的参数值,<br/>
	 * 如果无值或为空字串直接返回值，否则对字符串使用URLDecoder转为UTF-8编码.
	 * 
	 * @param key
	 *            .
	 * @return string.
	 */
	public String getStringURLDecodeToUTF8(String key) {
		...
		if (StringUtils.isBlank(value)) {
			...
			try {
				...
				LOG.warn("== getStringURLDecodeToUTF8 exception:", e);
				return "";
			}
		}
	}
	...
	public Map<String, Object> getParamMap() {
		...
		Set keys = map.keySet();
		for (Object key : keys) {
			parameters.put(key.toString(), getHttpRequest().getParameter(key.toString()));
		}
		return parameters;
	}
