...
public class BeanCloneHelper {
     ...
     * @return 返回复制后的对象
     */
    public static Object clone(Object value, int level) throws Exception {
        if (value == null) {
            return null;
        }
        if (level == 0) {
        ...
        if (value instanceof Collection) {//复制新的集合
            ...
            for (Object v : (Collection) value) {
                collection.add(clone(v, level));//深度复制
            }
            value = collection;
            ...
            if (c.equals(int[].class)) {
                ...
                Object[] objects = Arrays.copyOf(old, old.length, old.getClass());
                for (int i = 0; i < old.length; i++) {
                    objects[i] = clone(old[i], level);
                }
                value = objects;
            }
     ...
     * @return 复制后的对象，只复制一层
     */
    public static Object clone(Object value) throws Exception {
     ...
     * @throws Exception
     */
    public static Object deepClone(Object value) throws Exception {
        return clone(value, -1);
    }
}
